...
if trim(request("status"))<>"" then status=" and approval="&trim(request("status"))
			...
			set rs=server.CreateObject("adodb.recordset")
			rs.Open "select * from contracts where id>0"&full_name&passportdate1&date2&status&" order by datediff(d,'"&now()&"',start_date),approval desc",conn,1,1
			...
			if not rs.eof then
      				...
      				if (currentpage-1)*MaxPerPage>totalput then
       				...
       				if currentPage=1 then
            			showContent
            			showpage1=showpage(totalput,MaxPerPage,"manage_attorn_creditor_right.asp","&full_name="&trim(request("full_name"))&"&passport="&trim(request("passport"))&"&repayment_date="&trim(request("repayment_date"))&"&repayment_date2="&trim(request("repayment_date2"))&"&status="&trim(request("status")))
       				else
          				if (currentPage-1)*MaxPerPage<totalPut then
