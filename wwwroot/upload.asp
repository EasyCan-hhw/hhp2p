<!--#include file="inputUpload/UpLoad_Class.asp"-->
<%
 ...
 Upload.SingleSize=clng(1000 * 1024)*1024  '设置单个文件最大上传限制,按字节计；默认为不限制，本例为512M
 Upload.MaxSize=clng(5000 * 1024)*1024 '设置最大上传限制,按字节计；默认为不限制，本例为1G
 Upload.Exe="JPG|GIF|PNG|PDF"  '设置允许上传的扩展名
 Upload.GetData()
 if Upload.ErrorID>0 then
 	...
 	if Upload.files(-1).count>0 then
        ...
        upload.setApp "faild",1,0,"未上传任何文件"
 	end if
 end if
