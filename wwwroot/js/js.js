$(document).ready(function() {
	...
	$("#add_company_submit").click(function() {
		...
		if ($.trim($("#superior_companys").val()).length == 0) {
		...
		if (tosubmit) {
			...
			if (superior = true) {
				query.company_count = escape($("#superior_companys").val());
			} else {
				query.company_count = escape("0");
			}
			query.action = "add";
			$.ajax({
	...
	$("#add_work_application").click(function() {
		...
		if (tosubmit) {
			...
			query.mwork_input_hour = escape($("#input_hour").val());
			query.action = "add";
	...
	$("#add_user_submit").click(function() {
		...
		if ($.trim($("#user_code").val()).length == 0) {
			tosubmit = false;
			$("#user_code_err").html("请选择员工状态");
		}
		if ($.trim($("#company_id").val()).length == 0) {
			tosubmit = false;
			$("#company_id_err").html("请选择所属分公司");
		}
		/*if ($.trim($("#company_porportion").val()).length == 0) {
			tosubmit = false;
			$("#company_porportion_err").html("请选择正确的下属公司");
		}*/
		if ($.trim($("#company_porportion").val()).length == 0) {
			mycompany = false;
		}
		...
		if (tosubmit) {
			...
			query.company_id = escape($("#company_id").find("option:selected").attr("unval"));
			query.user_code = escape($("#user_code").val());
			query.job_id = escape($("#job_id").val());
			if (mylead_user) {
				query.lead_user = escape($("#lead_user").val());
			}else {
				query.lead_user = escape(0);
			}
			if (mycompany) {
				query.company_porportion = escape($("#company_porportion").val());
			}else {
	...
	$("#insert_approval_money_submit").click(function() {
		
		var tosubmit = true;
		if ($.trim($("#approval_date").val()).length == 0) {
			tosubmit = false;
			alert("错误");
		}
		if ($.trim($("#approval_photo").val()).length == 0) {
			tosubmit = false;
			alert("请上传图片");
		}
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (parseInt(data.split("|")[0]) == 1) {
						...
						alert("已提交");
						window.location.href = "manage_request_money.asp";
					}
				}
			});
		}
	...
	$("#add_creditor_right_submit").click(function() {
		...
		if ($.trim($("#advance_pay_creditorRight").val()).length == 0 || $.trim($("#month_pay_creditorRight").val()).length == 0) {
			tosubmit = false;
			$("#advance_pay_creditorRight_err").html("请必须填写该信息");
		}
		...
		if (tosubmit) {
			...
			query.advance_pay = escape($("#advance_pay_creditorRight").val());
			query.month_pay = escape($("#month_pay_creditorRight").val());
			query.other_other=escape($("#other_other").val());
	...
	$(".request_money_approval").click(function() {
		if (confirm('是否确定审批？')) {
			var id = this.id.split("_")[3];
			var query = new Object();
			query.id = id;
			...
			$.ajax({
				...
				success: function(data) {
					if (parseInt(data.split("|")[0]) == 1) {
						...
						alert("审批成功！");
						window.location.href = "approval_request_money.asp";
					}
				}
			});
		}
	...
	$("#request_money_approval").click(function() {
		if (confirm('是否确定审批？')) {
			var tosubmit = true;
			if (tosubmit) {
				$("#request_money_approval").attr("disabled", true);
				var query = new Object();
				query.id = escape($("#id").val());
				query.action = "approval";
				$.ajax({
