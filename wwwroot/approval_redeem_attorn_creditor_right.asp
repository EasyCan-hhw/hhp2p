...
<div id="content">
    ...
    <div id="content-header">
        <div id="breadcrumb"> <a href="####" class="tip-bottom"><i class="icon-th-list"></i> 赎回债权转让审批</a></div>
    </div>
    <!--End-breadcrumbs-->
...
if trim(request("status"))<>"" then status=" and approval="&trim(request("status"))
				...
				err_txt="<tr><td colspan=""9"">没有待审批的赎回债权转让</td></tr>"
			set rs=server.CreateObject("adodb.recordset")
			rs.Open "select * from contracts where redeem=1 and approval_redeem=0"&full_name&passportdate1&date2&status&" order by datediff(d,'"&now()&"',start_date),approval_redeem desc",conn,1,1
			...
			if not rs.eof then
      				...
      				if (currentpage-1)*MaxPerPage>totalput then
       				...
       				if currentPage=1 then
            			showContent
            			showpage1=showpage(totalput,MaxPerPage,"approval_redeem_attorn_creditor_right.asp","&full_name="&trim(request("full_name"))&"&passport="&trim(request("passport"))&"&repayment_date="&trim(request("repayment_date"))&"&repayment_date2="&trim(request("repayment_date2"))&"&status="&trim(request("status")))
       				else
          				if (currentPage-1)*MaxPerPage<totalPut then
			...
			%>
                               ...
                               <td style="vertical-align: middle; text-align:center">
                                   ...
                                   <a href="javascript:;" id="redeem_attorn_creditor_right_approval_<%=int(rs("id"))%>" class="redeem_attorn_creditor_right_approval">审批</a>
            ...
            %>
                        ...
                        <div style="margin-bottom: 20px;">
                            <span class="icon" style="cursor: pointer;" id="redeem_attorn_creditor_right_approvals"> <i class="icon-tags"></i> 批量审批</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        </div>
