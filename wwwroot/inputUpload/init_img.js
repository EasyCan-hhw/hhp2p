...
AjaxProcesser.prototype.addFile=function(){
	if(this.FileCount>=this.MaxFileCount){alert("超过最大允许上传的文件数量。\n\n最多允许上传" + this.FileCount+"个文件。" );
		return;
	}
	_this=this;
	...
	file.size=40;
	file.style.cssText="margin-bottom:10px;";
	...
	remove.value="移除";
	remove.type="button";
	remove.style.cssText="margin:0 0 10px 20px;";
...
AjaxProcesser.prototype.createUploader=function(){
	...
	frm.style.cssText="padding:10px;margin:0px;";
	var file=document.createElement("input");
	file.type="file";
	file.name="file" + getID();
	file.size=40;
	...
	button.style.cssText="margin:0 0 10px 20px;";
	...
	if(this.MaxFileCount>1){
		...
		add.value="添加文件";
		add.type="button";
		add.style.cssText="margin:0 0 10px 20px;";
...
AjaxProcesser.prototype.getProcess=function(){
	_this=this;
	Ajax({
		url:"inputupload/getProcess.asp?processid=" + this.processID,
		method:"get",
		dataType:"json",
		asc:true,
...
AjaxProcesser.prototype.getInformation=function(info){
	var uploadInfo={
		...
		total:info.total,
		cur:info.now,
		speed:reSize(parseInt(info.now/((Date.parse(new Date())-this.startTime)/1000))) + "/s",
	...
	case "uploading":
		...
		break;
	case "uploaded":
		uploadInfo.alt="上传, 服务器处理数据...";
		uploadInfo.stepId=3;
		break;
	case "processing":
		uploadInfo.alt="文件正在处理中: " + info.description;
		uploadInfo.stepId=4;
		break;
	case "processed":
		uploadInfo.alt="数据处理完成，准备保存文件...";
		uploadInfo.stepId=5;
		break;
	case "saving":
		uploadInfo.alt="保存文件: " + info.description;
		uploadInfo.stepId=6;
		break;
	case "saved":
		uploadInfo.alt="保存该文件已经完成，上传成功！";
