...
function showUpload(obj, inputID, Path, fcount, func) {
	if (!isInit) {
		var cont = "<div id=\"upload_Box\" style=\"z-index:999;position:absolute;border:1px #005EAC solid;padding:1px;width:455px;display:none;background-color:#fff;font-size:12px;\">";
		cont += "<div id=\"uploader_Title\" style=\"background-color:#004280 ;padding:6px; color:#FFF;\"><span style=\"float:right;cursor:pointer;\" onclick=\"this.parentNode.parentNode.style.display='none';Cover.hide();\">×</span>选择图片文件</div>";
		cont += "<div id=\"uploadContenter\" style=\"\"></div>";
		cont += "<iframe style=\"display:none;\" name=\"AnUploader\"></iframe>";
		cont += "</div>";
	...
	AjaxUp.savePath = (Path.trim() != "" ? Path.trim() : "upload");
	AjaxUp.createUploader();
...
var Cover = window.Cover = {
	...
	init: function(obj) {
		...
		this.ele.style.backgroundColor = "#ddd";
...
var _Ajax = window.Ajax = function(options) {
	...
	u = b ? u + "&aienrnd=" + Ajax_Rnd() : u + "?aienrnd=" + Ajax_Rnd();
...
AjaxProcesser.prototype.getProcess = function() {
	...
	Ajax({
		...
		succeed: function(msg, o) {
			...
			var img = "∵∴";
			if (pro.finish) {
				img = "<span style=\"font-weight:bold;color:green;\">√ ";
			}
			if (pro.step == "faild") {
				img = "<span style=\"font-weight:bold;color:red;\">×";
			}
