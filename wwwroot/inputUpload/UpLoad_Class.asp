	...
	Private Function GetErr(ByVal Num)
		Select Case Num
			Case 0
				GetErr = "处理数据！"
			Case 1
				GetErr = "数据大小超过" & GetSize(vMaxSize) & "限制！"
			Case 2
				GetErr = "未设置上传表单enctype属性为multipart/form-data或者未设置method属性为Post,上传无效!"
			Case 3
				GetErr = "包含非法扩展名(" & vErrExe & ")！\n\n只能上传扩展名为" & Replace(vExe, "|", ",") & "的文件！"
			Case 4
				GetErr = "对不起,程序不允许使用相同name属性的文件域!"
			Case 5
				GetErr = "上传单个文件大小限制超过" & GetSize(vSingleSize) & "！"
		End Select
	End Function
...
UploadFile.prototype.SaveToFile=function(){
	...
	try{
		...
		tmpStrm.close();
		tmpStrm = null;
		return {error:false,description:'Save the file successfully'};
	}catch(ex){
